...
do
  ...
  then
    ...
    then
      ...
      id1=$($PSQL "SELECT team_id FROM teams WHERE name='$WINNER'")
      id2=$($PSQL "SELECT team_id FROM teams WHERE name='$OPPONENT'")

      # insert games
      INSERT_GAME_RESULT=$($PSQL "INSERT INTO games(year, round, winner_id, opponent_id, winner_goals, opponent_goals) VALUES($YEAR, '$ROUND', $id1, $id2, $WINNER_GOALS, $OPPONENT_GOALS)")
